    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            if not shared.args.cfg_cache:
                logger.error(
                    "Please enable the cfg-cache option to use CFG with ExLlama_HF."
                )
                return
        ...
        if labels is None:
            ...
            logits = self.ex_model.forward(
                seq_tensor[-1:].view(1, -1), ex_cache, lora=self.lora
            ).to(input_ids.device)
        else:
            ex_cache.current_seq_len = 0
            logits = self.ex_model.forward(
                seq_tensor.view(1, -1), ex_cache, last_id_only=False, lora=self.lora
            )
